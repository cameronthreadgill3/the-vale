...
export function playerSpritePreviewUrl(classId: ClassId): string {
  ...
  // Transparent 1×1 until ready; callers that need sync get HTMLCanvas path above.
  return "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
}

/** Warm all class sheets + previews (call on boot / class select mount). */
export function preloadAllPlayerSprites(): void {
  const ids: ClassId[] = [
    "warden",
    "thornblade",
    "pathfinder",
  ...
  for (const id of ids) {
    void preloadPlayerSprite(id);
    void playerSpritePreviewUrl(id);
  }
}
