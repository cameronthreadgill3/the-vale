...
export function resolveQuestObjective(
  ...
  if (isChoirRemembersActive(log)) {
    const q = getChoirRemembersQuest(log);
    if (q) return objectiveForChoirRemembers(q, player, enemies, folk, map);
  }
  if (isCoilActive(log)) {
...
export function objectiveForEdgeRemembers(
  ...
  const onEdge = map.continentId === "thornreach" && map.kind === "overworld";
  ...
  if (!q.identifiedVole) {
    if (onEdge) {
      ...
      return {
        label: "Find Ash-vole (Thornreach)",
        x: player.x + TILE * 6,
        y: player.y - TILE * 4,
        kind: "landmark",
      };
    ...
    if (home) {
      return {
        label: "Gate to Ash-vole (Thornreach)",
        x: (home.x + 0.5) * TILE,
        y: (home.y + 0.5) * TILE,
        kind: "landmark",
      };
  ...
  if (!q.houndDone) {
    if (onEdge) {
      ...
      return {
        label: "Find Bark Hound (Thornreach)",
        x: player.x + TILE * 6,
        y: player.y - TILE * 4,
        kind: "landmark",
      };
    ...
    if (home) {
      return {
        label: "Gate to Bark Hound (Thornreach)",
        x: (home.x + 0.5) * TILE,
        y: (home.y + 0.5) * TILE,
        kind: "landmark",
      };
...
export function objectiveForChoirRemembers(
  ...
  if (q.status !== "active") return null;
  const rook = folk.find((f) => f.id === "rook");
  const onChoir = map.continentId === "sunken-choir" && map.kind === "overworld";

  if (!q.reachedChoir && !onChoir) {
    return objectiveTowardSunkenChoir(player, map, {
      dock: "Sail to Sunken Choir",
      landing: "Choir Landing",
      gate: "Gate to Sunken Choir",
      viaMist: "Mistmere gate / coastal dock",
      fallback: "Find a coastal dock toward the Choir",
    });
  }
  ...
  if (!q.identifiedMite) {
    if (onChoir) {
      ...
      return {
        label: "Find Briar Mite (Sunken Choir)",
        x: player.x + TILE * 6,
        y: player.y - TILE * 4,
        kind: "landmark",
      };
    ...
    return objectiveTowardSunkenChoir(player, map, {
      dock: "Sail to Briar Mite (Choir)",
      landing: "Choir Landing / Briar Mite",
      gate: "Gate to Briar Mite (Choir)",
      viaMist: "Mistmere gate / coastal dock",
      fallback: "Find Briar Mite (Sunken Choir)",
    });
  }
  ...
  if (!q.houndDone) {
    if (onChoir) {
      ...
      return {
        label: "Find Bark Hound (Sunken Choir)",
        x: player.x + TILE * 6,
        y: player.y - TILE * 4,
        kind: "landmark",
      };
    ...
    return objectiveTowardSunkenChoir(player, map, {
      dock: "Sail to Bark Hound (Choir)",
      landing: "Choir Landing / Bark Hound",
      gate: "Gate to Bark Hound (Choir)",
      viaMist: "Mistmere gate / coastal dock",
      fallback: "Find Bark Hound (Sunken Choir)",
    });
  }
  ...
  const toNightHome = continentGateOnMap(map, "nightglass-coast");
  if (toNightHome) {
    return {
      label: "Gate toward Rook (Nightglass)",
      x: (toNightHome.x + 0.5) * TILE,
      y: (toNightHome.y + 0.5) * TILE,
      kind: "landmark",
    };
  }
...
export function objectiveForMereRemembers(
  ...
  if (q.status !== "active") return null;
  const rook = folk.find((f) => f.id === "rook");
  const onMere = map.continentId === "mistmere" && map.kind === "overworld";

  if (!q.reachedMistmere && !onMere) {
    return objectiveTowardMistmere(player, map, {
      dock: "Sail to Mistmere",
      pier: "Mistmere Pier",
      gate: "Gate to Mistmere",
      viaCoast: "Coastal path toward Mistmere",
      fallback: "Find a Mistmere gate / path",
    });
  }
  ...
  if (!q.identifiedMite) {
    if (onMere) {
      ...
      return {
        label: "Find Briar Mite (Mistmere)",
        x: player.x + TILE * 6,
        y: player.y - TILE * 4,
        kind: "landmark",
      };
    ...
    return objectiveTowardMistmere(player, map, {
      dock: "Sail to Briar Mite (Mistmere)",
      pier: "Mistmere Pier / Briar Mite",
      gate: "Gate to Briar Mite (Mistmere)",
      viaCoast: "Coastal path toward Mistmere",
      fallback: "Find Briar Mite (Mistmere)",
    });
  }
  ...
  if (!q.houndDone) {
    if (onMere) {
      ...
      return {
        label: "Find Bark Hound (Mistmere)",
        x: player.x + TILE * 6,
        y: player.y - TILE * 4,
        kind: "landmark",
      };
    ...
    return objectiveTowardMistmere(player, map, {
      dock: "Sail to Bark Hound (Mistmere)",
      pier: "Mistmere Pier / Bark Hound",
      gate: "Gate to Bark Hound (Mistmere)",
      viaCoast: "Coastal path toward Mistmere",
      fallback: "Find Bark Hound (Mistmere)",
    });
  }
...
function objectiveTowardSunkenChoir(
  player: { x: number; y: number },
  map: WorldMap,
  labels: {
    dock: string;
  ...
  const coastal = dockTowardContinent(map, "sunken-choir");
  if (coastal) {
    return {
      label: labels.dock,
      x: (coastal.x + 0.5) * TILE,
  ...
  const gate = continentGateOnMap(map, "sunken-choir");
  if (gate) {
    return {
      label: labels.gate,
      x: (gate.x + 0.5) * TILE,
...
function objectiveTowardMistmere(
  player: { x: number; y: number },
  map: WorldMap,
  labels: {
    dock: string;
  ...
  const coastal = dockTowardContinent(map, "mistmere");
  if (coastal) {
    return {
      label: labels.dock,
      x: (coastal.x + 0.5) * TILE,
