...
- Class flavor: Warden/Thornblade melee, Patfainder bolts, Hearthmage/Verdant magic (Verdant soft heal-on-kill), Hollowborn short-range hybrid
...
### Fourth sticky quest — Mistmere Path

After *Hollow Watch* completes, Rook starts **Mistmere Path** — the world opens past the hollow toward Mistmere.

1. **Talk to Rook** for the Mistmere handoff (Survive · Learn · Progress)
2. **Reach the Mistmere gate** on Thornreach (yellow arrow; stand on / use the gate — crossing is optional)
3. **Report back to Rook** to close the path

Rewards: ~45g + ~90 combat XP + distance skill XP. No new creature art — reuses existing gates and folk. First Story / Accession only.

### Wayfinding (where to go)

New cues so you are never lost on Thornreach:

1. **Quest arrow + HUD line** — Active sticky quests show a yellow edge/on-screen arrow toward the current objective (cairns, hollow entrance, Mistmere gate, Shade Wisp, Needle Rat, Bark Hound, or Rook) plus a HUD line like `→ Identify Needle Rat · 12 tiles`.
